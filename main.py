...
app = Flask(__name__)
openai.api_key = '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'
    ...
    if 'sentence' in request.form and 'modsentence' in request.form:
        ...
        if 0 <= sentence_number < len(outline_parts):
            ...
            for i, part in enumerate(outline_parts):
                prompt += f"{i+1}. {part}\n"
            prompt += f"\nPlease rewrite the following part with more creativity but maintain same plot as previously, detail, and specificity, while maintaining the same genre and characters, and without altering the setting. Keep the narrative engaging and avoid generic descriptions.\nPart to modify: {modsentence}"
